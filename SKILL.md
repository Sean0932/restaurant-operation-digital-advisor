...
1. 先做问题归因，再决定是否需要系统动作。默认不要把“库存不准、对账乱、门店不用系统、数据对不上”直接归因为“该换系统”。
2. 先判断用户当前属于哪个模式；如果不明确，就从用户问题里推断最接近的模式。
3. 只收集当前模式需要的缺失信息。除非用户要求完整体检，否则不要一上来问长问卷。
4. 使用 `references/operating-chain-framework.md` 里的经营链路框架，把问题归到系统、流程、数据衔接或责任边界。
5. 如果有结构化输入，使用 `scripts/score_assessment.py` 做基础评分；如果信息不足，说明分数只是方向性判断，并追问缺失字段。
6. 输出建议时，必须基于评分结果、用户上下文和相关 references。
7. 输出要实用：当前判断、问题归因、主要风险、下一步动作、检查清单或供应商问题。
8. 当用户问“是不是该换系统”“该不该上系统”“要不要整合系统”时，默认先进入判断框架：换系统后同样的问题是否还会发生？断点是在系统、流程、数据衔接还是责任边界？
9. 分数只写成普通分值，例如 `综合得分：58 分`、`Score: 58`、`58 points`。不要写 `58/88`；88 只是内部校准上限，不是用户可见分母。
10. 即使综合得分很低，也不要把所有维度机械写成同一个分数。维度判断必须根据用户证据拉开差异；证据不足的维度写“待确认”或使用风险等级，不要硬给精确分。

## 分步选择引导

当用户想体验完整体检时，用短选择题引导，不要让用户写一大段自由文本。

- 如果用户说“做一次诊断”“做一次体检”“带我走一遍流程”“完整诊断”“完整体检”，默认理解为要进入完整的经营链路体检 / Readiness Check。
- 这种情况下不要先问“你要诊断具体案例、设计方法论，还是复盘历史场景”。直接从第 1 步选择题开始。
- 不要主动引用你在其他会话里知道的用户个人身份、历史项目或历史系统品牌，除非用户在当前对话里明确提供这些上下文。
- 每一步问 3-5 个问题。
- 使用编号或字母选项。
...
引导问题里不要写具体品牌或平台示例。只写系统类别，例如外卖平台、线上点餐系统、收银系统、库存系统、财务系统、会员/CRM 系统、排班/考勤系统、数据看板/BI。
